...
///////////////////////////////////////////////
static int DS1307_dectobcd( int );
static int DS1307_bcdtodec( int );
static int DS1307_hilow_check( int, int, int);
///////////////////////////////////////////////
...
int DS1307_bulkRead( int addr, int quantity, char *data) {
    ...
    test = ezI2CWriteBulk((uint8_t *)temp_data,2);
    if (test < 0) return (1);  // the write operation failed
    //JEM ds1307i2c.stop();   // now the DS1307 is pointing to the first register
    if ( addr != 0 ) test = ezI2CReadBulk((uint8_t *)temp_data,addr);  // now the DS1307 address pointer is pointing to correct address
    if (test < 0) return (1);  // the read operation failed
    test = ezI2CReadBulk((uint8_t *)data,quantity);   // read the DS1307 registers now
    if (test < 0) return (1);  // read operation failed
    return(0);  // looks like the data read was good
}

int DS1307_read(int addr, int *data) {
    // addr must be 0 - 62 as the 64th(or 63rd as indexed from 0) ram location is clobered in this method of access
    int test = 0;
    char temp_data[65];
    test = DS1307_bulkRead(addr, 1, &temp_data[0]);
    if (test < 0) return(1);   // fail because read to DS1307 failed
    *data = (int)temp_data[0];      // returing the read data by pointer
    return (0);                     // the single read is successfull
}

int DS1307_bulkWrite( int addr, int quantity, char *data) {
    // note the char array at data must contain 63 locations or unpredictable behavior will happen
    // addr must be 0 - 62 as the 64th(or 63rd as indexed from 0) ram location is clobered in this method of access
    // quantity must be 1 - 63 as the 64th ram location is clobered in this method of access
    int test = 0 ;
    ...
    test = ezI2CWriteBulk((uint8_t *)temp_data, (quantity + 1));
    //JEM ds1307i2c.stop();
    return(test >= 0 ? 0 : 1);   // 0 for success 1 for failure to write
}

int DS1307_write( int addr, int data ) {
    // addr must be 0 - 62 as the 64th(or 63rd as indexed from 0) ram location is clobered in this method of access
    int test = 0 ;
    char temp_data[2] ;
...
int DS1307_startClock(void) {           // start the clock
    ...
    test = DS1307_read(DS1307_sec, &junk);
    if (test == 1) return(1);       // fail because read to DS1307 failed
    junk = ( 0x7F & junk);              // basicaly i mask bit 8 to set it to zero
    test = DS1307_write(DS1307_sec,junk);        // now write the seconds back to register and because bit 8 is zero this starts clock.
    if (test == 1) return(1);        // fail because read to DS1307 failed
    return(test);                        //
}
...
int DS1307_stopClock(void) {           // stop clock
    ...
    test = DS1307_write(DS1307_sec,junk);        // now write the seconds back to register and because bit 8 is one this starts clock.
    if (test == 1) return(1);        // fail because read to DS1307 failed
    return(test);                        //
}
...
int DS1307_twelveHour(void) {           // set 12 hour mode
    ...
    test = DS1307_read(DS1307_hour, &junk);
    if (test == 1) return(1);       // fail because read to DS1307 failed
    ...
    test = DS1307_write(DS1307_hour,(0x40 | junk));    // set bit 6 with the new 12 hour time converted from the 24 hour time
    if (test == 1) return(1);       // fail because read to DS1307 failed
...
int DS1307_twentyFourHour(void) {     // set 24 hour mode
    ...
    test = DS1307_write(DS1307_hour,( 0xBF & junk));   // clear bit 6 and set the new 24 hour time converted from 12 hour time
    if (test == 1) return(1);       // fail because read to DS1307 failed
...
int DS1307_setTime(int sec, int min, int hour, int day, int date, int month, int year) {        // to set the current time and start clock
    ...
    if (1 == DS1307_hilow_check( 59, 0, sec)) {
        ...
        if (1 == (DS1307_write(DS1307_sec,DS1307_dectobcd(sec)))) return(1);    // failed to write for some reason
    }

    if (1 == DS1307_hilow_check( 59, 0, min)) {
        return(1);    // failed because recieved value is not in bounds
    } else {
    ...
    if (1 == DS1307_hilow_check( 7, 1, day)) {
        ...
        if (1 == (DS1307_write(DS1307_day,DS1307_dectobcd(day)))) return(1);    // failed to write for some reason
    }

    if (1 == DS1307_hilow_check( 31, 1, date)) {
        return(1);    // failed because recieved value is not in bounds
    } else {
        if (1 == (DS1307_write(DS1307_date,DS1307_dectobcd(date)))) return(1);    // failed to write for some reason
    }

    if (1 == DS1307_hilow_check( 12, 1, month)) {
        return(1);    // failed because recieved value is not in bounds
    } else {
        if (1 == (DS1307_write(DS1307_month,DS1307_dectobcd(month)))) return(1);    // failed to write for some reason
    }

    if (1 == DS1307_hilow_check( 99, 0, year)) {
        return(1);    // failed because recieved value is not in bounds
    } else {
...
int DS1307_getTime(int *sec, int *min, int *hour, int *day, int *date, int *month, int *year) { // to get the current time information
    ...
    *sec = DS1307_bcdtodec( *sec);                         // bcd is now dec value

    if (1 == DS1307_read(DS1307_min,min)) return(1);       // failed to read for some reason
    *min = (*min & 0x7F );                                  // drop bit 7 because it should be 0 anyways
    *min = DS1307_bcdtodec( *min);                         // bcd is now dec value
    ...
    if (1 == DS1307_read(DS1307_day,day)) return(1);       // failed to read for some reason
    *day = (*day & 0x07 );                                  // drop the non used bits
    *day = DS1307_bcdtodec( *day);                         // bcd is now dec value

    if (1 == DS1307_read(DS1307_date,date)) return(1);     // failed to read for some reason
    *date = (*date & 0x3F );                                // drop bit 6 and 7 not used for date value
    *date = DS1307_bcdtodec( *date);                       // bcd is now dec value

    if (1 == DS1307_read(DS1307_month,month)) return(1);   // failed to read for some reason
    *month = (*month & 0x1F );                              // drop bit 5, 6 and 7 not used for month value
    *month = DS1307_bcdtodec( *month);                     // bcd is now dec value

    if (1 == DS1307_read(DS1307_year,year)) return(1);     // failed to read for some reason
    *year = DS1307_bcdtodec( *year);                       // bcd is now dec value

    return (0);                                             // data returned is valid
}


int DS1307_dectobcd( int dec) {
    int low = 0;
    int high = 0;
...
int DS1307_hilow_check( int hi, int low, int value) {
